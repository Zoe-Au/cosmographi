...
class LogPosterior(ck.Module):
    def __init__(self, key, y, y_err, start_time, end_time, obs_t, instrument, band: jnp.ndarray, exp_time: jnp.ndarray, sky_brightness: jnp.ndarray, PSF_Aeff: jnp.ndarray, temp=1, name: str=None, z: float=None, blackhole_mass: float=None, accretion_rate: float=None, inclination_angle: float=None, rin_to_rng: float=None, perturbations: jnp.ndarray=None, luminosity_density_scaling: float=None, A_V_c00mw: float=None, R_V_c00mw: float=None, alpha: float=None, A: float=None, **kwargs):
        ...
        super().__init__(name)
        self.log_likelihood = LogLikelihoodAGN(name= "log_likelihood", y=y, y_err=y_err, obs_t=obs_t, end_time=end_time, start_time=start_time, z=z, instrument=instrument, band=band, exp_time=exp_time, sky_brightness=sky_brightness, PSF_Aeff=PSF_Aeff, blackhole_mass=blackhole_mass, accretion_rate=accretion_rate, inclination_angle=inclination_angle, rin_to_rng=rin_to_rng, perturbations=perturbations, luminosity_density_scaling=luminosity_density_scaling, A_V_c00mw=A_V_c00mw, R_V_c00mw=R_V_c00mw) 
...
class AGNFit(ck.Module):
    ...
    def plot(self, true_values: dict[str, any] = None):
        ...
        if true_values is not None:
            lines = [(name, {}, true_values[name]) for name in var_names if name in true_values and name != "perturbations"]
        ...
        if true_values is not None:
            ...
            for name in var_names:
                if name == "perturbations" and name in true_values:
        ...
        for name in var_names:
            if name != "perturbations":
                ...
                data = idata.posterior["perturbations"]
                n_elements = data.sizes["samples"]
                true_array = None
                if true_values is not None and "perturbations" in true_values:
                    true_array = np.asarray(true_values["perturbations"]).ravel()
                for i in range(n_elements):
                    scalar_name = f"perturbations[{i}]"
